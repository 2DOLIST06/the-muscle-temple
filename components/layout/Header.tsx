...
export function Header() {
  ...
  const searchForm = (mobile = false) => (
    <form onSubmit={submitSearch} role="search" className={mobile ? 'w-full' : 'mx-auto w-full max-w-2xl'}>
      ...
      <div className="flex h-12 items-center rounded-full border-2 border-slate-300 bg-slate-100 px-4 transition focus-within:border-brand-500 focus-within:bg-white focus-within:ring-4 focus-within:ring-brand-50">
  ...
  return (
    <header className="sticky top-0 z-50 border-b-2 border-slate-300 bg-white shadow-[0_4px_20px_rgba(15,23,42,0.10)]">
      <Container>
        ...
        <div className="hidden border-t-2 border-slate-200 py-2.5 lg:flex lg:justify-center">
          <nav className="flex items-center rounded-full border border-slate-200 bg-slate-100 p-1 text-sm font-semibold text-slate-600" aria-label={locale === 'fr' ? 'Navigation principale' : 'Main navigation'}>
            {navigation.map((item) => (
        ...
        {isMenuOpen ? (
          <div id="mobile-navigation" className="border-t-2 border-slate-200 pb-5 pt-3 lg:hidden">
