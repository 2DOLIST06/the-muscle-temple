...
## Payload v3 et migrations PostgreSQL (Render + Vercel)

Le projet utilise Payload v3 avec des slugs de collections en minuscules/pluriel (`users`, `authors`, `posts`, `categories`, `media`).
La base existante contient des tables historiques en `PascalCase` (`User`, `Post`, etc.) qui **ne doivent pas être modifiées**.
...
### Procédure recommandée en production

1. Déployer le code sur Vercel (avec les scripts ci-dessus disponibles).
2. Ouvrir un shell sur le service Render qui héberge PostgreSQL (ou un job one-shot connecté avec la même `DATABASE_URL` de production).
3. Depuis ce shell (dans le repo à la bonne révision), lancer :
...
4. Vérifier ensuite :
   - `/debug-payload` retourne `ok: true`
   - `/api/posts` retourne un JSON Payload valide
   - `/cms` charge l'admin Payload
   - `/admin` (admin maison) reste inchangé

### Pourquoi Render shell plutôt que Vercel runtime

- Les migrations doivent être exécutées **une fois** et de façon contrôlée, au plus proche de la base PostgreSQL Render.
- Éviter d'exécuter des migrations au démarrage de fonctions serverless Vercel.
- Si besoin d'automatisation, préférer un job manuel/CI dédié avec token/secret plutôt qu'une route publique.

### Option temporaire (si aucun shell n'est disponible)

Créer une route privée de migration protégée par secret (ex: header `x-migration-token`), exécuter `payload migrate` une seule fois, puis supprimer immédiatement la route.
